        ...
        {users?.map((user: any) => (
          <div key={user.userId} className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Avatar>
                <AvatarImage src={user?.displayAvatarURL} alt={user.displayName} />
            ...
            <p className="text-sm font-medium text-white">Joined {new Date(user.joinedTimestamp).toLocaleDateString()}</p>
          </div>
        ))}
      </CardContent>
    </Card>
