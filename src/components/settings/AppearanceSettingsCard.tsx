...
import { barlowSemiCondensed, geistSans } from "@/app/:d:/app/server/[serverId]/fonts";
...
const themes: Theme[] = [
    {
        id: "mercury-2025",
        name: "Mercury",
        description: "Step into the world of the Mercury Minecraft server with an out-of-world theme.",
        color_1: "#FF8A00",
        color_2: "#A33B00",
        color_3: "#FF0000",
    ...
    },
    {
        id: "pride-2025",
        name: "Pride",
        description: "Showcase your support for the LGBTQIA+ community with this stellar theme.",
        color_1: "#FF0066",
        color_2: "#FFCC00",
        color_3: "#33CCFF",
    ...
    },
    {
        ...
        description: "Rep your love for Discord with this very, very, blurple theme. (We aren't allowed to use the Discord font)",
        color_1: "#5865F2",
        color_2: "#7289DA",
        color_3: "#99AAB5",
...
export const AppearanceSettingsCard = ({ currentServerData }: any) => {
    ...
    const [colorStop1, setColorStop1] = useState(currentServerData?.data.mmData.module_config.appearance?.gradient.color_1 || "#00BFFF");
    const [colorStop2, setColorStop2] = useState(currentServerData?.data.mmData.module_config.appearance?.gradient.color_2 || "#8A2BE6");
    const [colorStop3, setColorStop3] = useState(currentServerData?.data.mmData.module_config.appearance?.gradient.color_3 || "#FF0080");
    const [wallpaper, setWallpaper] = useState(currentServerData?.data.mmData.module_config.appearance?.background || "");
    const [overlayPercent, setOverlayPercent] = useState(currentServerData?.data.mmData.module_config.appearance?.overlay_percent || "0.6");
    const [selectedFont, setSelectedFont] = useState(currentServerData?.data.mmData.module_config.appearance?.font || "font-geist");

    const currentlySelectedPreset = currentServerData?.data.mmData.module_config.appearance?.theme_id !== "custom" ? themes.find(t => t.id === currentServerData?.data.mmData.module_config.appearance?.theme_id) : {
        background: currentServerData?.data.mmData.module_config.appearance?.background,
        font: currentServerData?.data.mmData.module_config.appearance?.font,
        name: "Your Theme",
        description: "Your very own custom theme!",
        color_1: currentServerData?.data.mmData.module_config.appearance?.color_1,
        color_2: currentServerData?.data.mmData.module_config.appearance?.color_2,
        color_3: currentServerData?.data.mmData.module_config.appearance?.color_3,
        overlayPercent: currentServerData?.data.mmData.module_config.appearance?.overlay_percent || "0.9",
    }
    ...
    const setTheme = async (theme: any) => {
        ...
        const response = await updateGuildSettings(currentServerData?.data.dsData.id, "appearance_set_theme", {
    ...
    const updateGradient = async () => {
        const response = await updateGuildSettings(currentServerData?.data.dsData.id, "appearance_mymod_gradient", {
            color_1: colorStop1,
            color_2: colorStop2,
            color_3: colorStop3
        })
    ...
    const updateWallpaper = async () => {
        const response = await updateGuildSettings(currentServerData?.data.dsData.id, "appearance_wallpaper", wallpaper)
    ...
    const updateFont = async () => {
        const response = await updateGuildSettings(currentServerData?.data.dsData.id, "appearance_font", wallpaper)
    ...
    return (
        <Card className={`w-full h-[100%] overflow-y-auto bg-black border-zinc-700 p-4`}>
            <div className="flex flex-col gap-2 items-start w-full text-white">
                <h4 className="text-xl text-left w-full font-bold text-zinc-300">Change how MYMOD looks</h4>
                <h4 className="text-sm text-left w-full font-medium text-zinc-500">All settings changed here affect <b>every member</b> that has access to your server's Dashboard.</h4>
                <h4 className="text-xl text-left w-full font-bold text-zinc-300">Currently selected preset</h4>
                <Card className="overflow-hidden w-[370] h-[200px] p-0 border border-zinc-900">
                    <div className="relative w-full h-full">
                        ...
                        <div className="absolute inset-0 z-[40] self-end flex flex-col items-start justify-start p-4 text-white">
                            <p className="text-lg font-semibold">{currentlySelectedPreset?.name}</p>
                            <p className="text-sm font-normal">{currentlySelectedPreset?.description}</p>
                        </div>
                    </div>
                </Card>
                <br />
                <h4 className="text-xl text-left w-full font-bold text-zinc-300">Presets</h4>
                <h4 className="text-sm text-left w-full font-medium text-zinc-500 py-2">Want to use a pre-made theme from MYMOD? Look no further. Click on a theme to view more information about the theme.</h4>
                        ...
                        themes.map((theme: any) => (
                            <Dialog key={theme.id}>
                                ...
                                <DialogContent className="dark min-w-[80vw] flex flex-col text-white">
                                    ...
                                    <br />
                                    <h4 className="text-sm uppercase text-left w-full font-bold text-zinc-500 py-2">What you get</h4>
                                    <div className="flex flex-row w-full items-start gap-2">
                                        <div className="flex flex-col w-full items-start gap-2">
                                            <h4 className="text-lg text-left w-full font-bold text-zinc-300">Gradient</h4>
                                        ...
                                        <div className="flex flex-col w-full items-start gap-2">
                                            <h4 className="text-lg text-left w-full font-bold text-zinc-300">Wallpaper</h4>
                                            <Card className={`overflow-hidden w-full h-[200px] bg-black p-0 border border-zinc-900`}>
                                                {theme?.background.endsWith(".mp4") && (
                                                    <>
                                                        <video
                                        ...
                                        <div className="flex flex-col w-full items-start gap-2">
                                            <h4 className="text-lg text-left w-full font-bold text-zinc-300">Font</h4>
                                            <Card className={`${theme.font} overflow-hidden w-full h-[200px] bg-black p-0 border border-zinc-900`}>
                                    ...
                                    <Button onClick={() => setTheme(theme.id)} variant="outline" className="w-auto self-end dark text-white">
                                        Apply theme
                                    </Button>
                                </DialogContent>
                            </Dialog>
                ...
                <br />
                <h4 className="text-xl text-left w-full font-bold text-zinc-300">Gradient Colors</h4>
                <h4 className="text-sm text-left w-full font-medium text-zinc-500 py-2">This setting changes the colors of the gradients you see in your Dashboard.</h4>
                <h4 className="text-xs text-left uppercase w-full font-medium text-zinc-500 py-2">Preview</h4>
                <div className="flex flex-row gap-1 items-start justify-between w-full">
                    ...
                    <div className="flex flex-col items-start gap-1">
                        ...
                        <Button onClick={updateGradient} variant="outline" size="icon" className="dark self-end text-white">
                            <Save />
                        </Button>
                    </div>
                </div>
                <br />
                <h4 className="text-xl text-left w-full font-bold text-zinc-300">Wallpaper</h4>
                <h4 className="text-sm text-left w-full font-medium text-zinc-500 py-2">This setting changes the wallpaper of your Dashboard. The wallpaper must be hosted somewhere where a link can be obtained. Video wallpapers (.mp4) are supported! Please note: Using video wallpapers may hinder performance.</h4>
                <h4 className="text-xs text-left uppercase w-full font-medium text-zinc-500 py-2">Preview</h4>
                <div className="flex flex-row gap-2 items-start justify-between w-full">
                    ...
                    <div className="flex flex-col gap-1 items-start justify-start">
                        ...
                        <Button onClick={updateWallpaper} variant="outline" size="icon" className="dark self-end text-white">
                            <Save />
                        </Button>
                    </div>
                </div>
                <br />
                <h4 className="text-xl text-left w-full font-bold text-zinc-300">System Font</h4>
                <h4 className="text-sm text-left w-full font-medium text-zinc-500 py-2">This settings changes the system font applied to your Dashboard.</h4>
                <div className="flex flex-row items-center justify-start gap-2">
                    ...
                    <Button onClick={updateFont} variant="outline" size="icon" className="dark self-end text-white">
                        <Save />
                    </Button>
                </div>
            </div>
