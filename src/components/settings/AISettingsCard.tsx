...
export const AISettingsCard = ({ currentServerData }: any) => {
  ...
  const [setting, setSetting] = useState<boolean>(
    currentServerData?.data?.mmData.module_config.mymod_intelligence.enabled,
  );
    ...
    <Card className="w-full h-[100%] overflow-y-auto bg-black border-zinc-700 p-4">
      <div className="flex flex-col h-full w-full items-center justify-center">
        <div className="relative w-12 h-12 m-8">
          {/* Glow behind icon */}
          <div
            className="absolute inset-0 rounded-full blur-lg opacity-90 animate-pulse"
            style={{
              background:
                "linear-gradient(135deg, #0090F7, #BA62FC, #F2416B, #F55600)",
            }}
          />
        ...
        <h4 className="text-xl text-center w-full font-bold text-zinc-300">
          MYMOD Intelligence
        </h4>
        <h4 className="text-sm text-center w-full font-medium text-zinc-500 p-2">
          An all new, personal intelligence system from ENDR, designed to make
          your work on MYMOD easier.
        </h4>
        <br />
